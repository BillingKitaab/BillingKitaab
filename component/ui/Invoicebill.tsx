...
const InvoiceForm = () => {
  ...
  const generatePDF = async (values: any, subtotal: number, tax: number, total: number, discountAmount: number = 0) => {
    ...
    if (planName === "Starter" || planName === "Free") {
      ...
      doc.text("Generated by BillingKitaab - Upgrade to remove watermark", 105, 290, { align: "center" });
    }
  ...
  const sendInvoice = async (values: any, subtotal: number, tax: number, total: number) => {
    ...
    if (uploadErr) {
      ...
      alert(`Note: Failed to upload PDF to cloud storage (${errMsg}). Database record will still be created.`);
    }
    ...
    // Auto-increment Next Invoice Number in state to prevent duplicate errors
    const match = values.invoiceNumber.match(/INV-(\d+)/);
    if (match && match[1]) {
