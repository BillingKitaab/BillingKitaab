...
import { FaEnvelope, FaLock, FaArrowRight, FaArrowLeft, FaGoogle, FaMobileAlt } from "react-icons/fa";
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ backgroundColor: "#2f2f33", opacity: transitioning ? 1 : 0 }}
      />

      <div className="min-h-screen w-full bg-white flex justify-center items-start pt-20 sm:pt-24 mb-10 sm:mb-0 font-sans px-4 sm:px-6">
        {/* Back Button */}
        <Link
          href="/"
        ...
        <div className="w-full max-w-[440px] sm:max-w-[600px] md:max-w-[650px] lg:max-w-[650px] bg-[#F5F6F7] rounded-2xl shadow-xl flex flex-col items-center justify-start px-6 sm:px-10 py-8">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-[#2f2f33] mb-1">Welcome Back</h2>
          <p className="text-sm text-gray-500 mb-6">Sign in to continue to BillingKitaab</p>
          ...
          {activeTab === "email" && (
            <form onSubmit={onEmailSubmit} className="w-full flex flex-col gap-4">
              ...
              >
                {loading ? "Logging in..." : "Login"} <FaArrowRight className="text-xs" />
              </button>
            </form>
          )}
            ...
            <div className="w-full flex flex-col gap-4">
              {!otpSent ? (
                <>
                  ...
                  <button
                    onClick={handleSendOtp} disabled={otpLoading || phone.length < 6}
                    className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all duration-300 ${phone.length >= 6 && !otpLoading ? "bg-[#2F2F33] text-white hover:bg-[#3a6f77]" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
                  >
                ...
                <>
                  ...
                  >
                    {otpLoading ? "Verifying..." : "Verify & Login"} <FaArrowRight className="text-xs" />
                  </button>
                  <button
          ...
          <div className="w-full flex justify-center mt-6 mb-2 text-sm text-[#2F2F33] font-sans">
            <span>Don&apos;t have an account? <Link href="/register" className="text-[#3a6f77] font-bold hover:underline">Sign Up</Link></span>
          </div>
        </div>
      </div>
    </>
